...
def delete_kb(purchase_data: List[Purchase], tastes: List[Taste]) -> InlineKeyboardMarkup:
    logger.error(purchase_data)
    kb = InlineKeyboardBuilder()
    for purchase in purchase_data:
        product = purchase.product
        if purchase.taste_id != 0:
            kb.button(text=f"{product.name} ({tastes[purchase.taste_id].taste_name}) - {product.price}₽", callback_data=f"item_dell_{purchase.id}")
        else:
